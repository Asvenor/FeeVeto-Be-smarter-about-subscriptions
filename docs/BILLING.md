...
- A **$49.99 lifetime upgrade Checkout is prepared but unpaid** for the same monthly test account. Lifetime hosted completion and automatic monthly cancellation are not yet verified with this purchase. SCA, calendar-month renewal, and portal UI/mobile/keyboard interaction also remain unverified. An earlier billing-anchor reset with no proration produced no new invoice, so it was not reported as renewal success; the passing API test checks a new paid subscription instead. Automated renewal/upgrade scenarios still pass with simulated Stripe responses.
