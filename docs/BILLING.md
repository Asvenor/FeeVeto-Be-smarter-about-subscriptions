...
- These API payments **did not complete hosted Checkout**. Monthly/lifetime hosted payment completion, lifetime-upgrade cancellation through a paid hosted session, SCA, calendar-month renewal, portal UI/mobile/keyboard interaction remain unverified. An attempted billing-anchor reset with no proration produced no new invoice in this sandbox, so it was not reported as renewal success; the passing test explicitly checks a new paid subscription instead. Automated renewal/upgrade scenarios still pass with simulated Stripe responses.
- Browser automation is blocked by an admin-policy verification failure. No alternate browser-control mechanism was used. Public Clerk/Stripe API tests are independent of browser testing, not evidence that the hosted screens work.
