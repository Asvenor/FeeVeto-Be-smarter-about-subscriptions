...
export async function curatedRecommendations(context, query, access, { load = loadPrivateCatalogue } = {}) {
  const catalogue = await load(context);
  return selectRecommendations(catalogue, query, { premiumAccess: access?.premiumAccess === true });
}
