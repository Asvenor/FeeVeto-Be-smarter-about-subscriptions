import { journeySubmission, journeyQuery } from "../../js/journeyModel.js";
...
export async function readJourneyBody(request) {
  ...
  try {
    ...
    throw new JourneyError(400, "A valid JSON object is required.");
  }
}
export async function journeyIdentity(
  context,
  {
    required = false,
    identityResolver = getVerifiedIdentity,
    paidAccessResolver = getPaidPremiumAccess,
  } = {},
) {
  if (!context.request.headers.get("authorization")) {
    if (required)
      throw new JourneyError(401, "Sign in to save or open account audits.");
    return { identity: null, access: resolveAccess() };
  }
  let identity;
  try {
    identity = await identityResolver(context);
  } catch {
    throw new JourneyError(
      503,
      "Account verification is unavailable. Your answers are kept; retry shortly.",
    );
  }
  if (!identity)
    throw new JourneyError(
  ...
  const paidPremiumAccess = await paidAccessResolver({
...
export async function createJourneyAssessment(
  context,
  body,
  access,
  ...
  try {
    ...
    alternatives = {
      ...
      items: [],
      message:
        "The catalogue could not be checked. Your answers are kept; retry to load comparisons.",
    };
  }
  return {
    draft,
    marketCurrency,
    assessment: assessJourney(draft, alternatives, { now, marketCurrency }),
  };
}
