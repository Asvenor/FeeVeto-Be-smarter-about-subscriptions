...
export async function getVerifiedAccess(
  ...
  if (!requestState.isAuthenticated) return resolveAccess();

  const auth = requestState.toAuth();
  if (!auth?.userId) return resolveAccess();

  const user = await clerk.users.getUser(auth.userId);
  const paidPremiumAccess = await paidAccessResolver({ userId: auth.userId, env: context.env });
