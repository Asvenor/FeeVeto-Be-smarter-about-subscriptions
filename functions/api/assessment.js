...
import { readJourneyBody,journeyIdentity,createJourneyAssessment } from '../_shared/journey-service.js';
...
export async function handleAssessmentRequest(context,options={}) {
  ...
  }catch(error){return json({error:error.status?error.message:'Assessment is temporarily unavailable. Your answers are kept; retry.'},{status:error.status || 503});}
}
